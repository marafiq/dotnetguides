...
public static class ImpulseGenerator
{
    ...
    {
        // Only generate route files for GET endpoints (views)
        if (!endpoint.IsQuery) return;
        ...
        var hasParams = endpoint.Route.Contains('{');
        ...
        if (endpoint.ResponseType != null)
        {
            ...
            if (hasParams)
            {
                var paramNames = ExtractRouteParams(endpoint.Route);
                sb.AppendLine($"    let url: string = RoutePaths.{routeName};");
                foreach (var param in paramNames)
                {
                    sb.AppendLine($"    url = url.replace('{{{param}}}', params.{param});");
                }
                sb.AppendLine($"    return impulse<{responseName}>(url);");
            }
            else
            {
                sb.AppendLine($"    return impulse<{responseName}>(RoutePaths.{routeName});");
            }
            sb.AppendLine("  },");
        }
