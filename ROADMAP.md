...
| Tool | Purpose | Why |
|------|---------|-----|
| **.NET 10** | Backend runtime | Latest Minimal API, TypedResults |
| **Bun** | JS runtime + package manager | 10x faster than Node/npm |
| **Vite** | Dev server + bundler | Fast HMR, optimized builds |
| **tsgo** | TypeScript compiler | 10x faster than tsc (Go-based) |
| **TanStack Router** | React routing | Type-safe, file-based routing |
| **Zod** | Validation | Runtime schemas from FluentValidation |
...
**Dev experience:**
- `dotnet run` starts both Kestrel + Vite (via Bun)
- React HMR via Vite
- API calls proxy to Kestrel
- File changes → tsgo compiles → Vite HMR updates
...
dotnet publish
    ↓
[bun run build] → Vite builds → dist/assets/
    ├── index-[hash].js
    ├── index-[hash].css
    └── .vite/manifest.json
    ↓
...
```json
// .vite/manifest.json (generated by Vite via Bun)
{
  "index.html": {
...
<Target Name="BuildClientApp" BeforeTargets="Publish">
  <Exec Command="bun run build" WorkingDirectory="ClientApp" />
</Target>
...
│       ├── index-e5f6g7h8.css
│       ├── chunk-vendor-xyz123.js  # Code-split chunks (Vite)
│       └── .vite/
│           └── manifest.json
└── appsettings.json
```
...
# 3. Restore and build sample (bun install happens via MSBuild)
dotnet restore samples/Impulse.Sample
dotnet build samples/Impulse.Sample

# 4. Run sample with hot reload (Vite starts via MSBuild)
...
dev: pack restore
	...
	@trap 'kill 0' EXIT; \
	dotnet watch run --project samples/Impulse.Sample & \
	cd samples/Impulse.Sample/ClientApp && bun run dev & \
	wait
...
```xml
<Project>
  <!-- Auto bun install if node_modules missing -->
